...
// API request handler for server-fixed.js
async function handleApiRequest(req, res) {
  const parsedUrl = url.parse(req.url, true);
  const pathSegments = parsedUrl.pathname.split('/').filter(Boolean);
  
  // Handle /api/lot/:lotNumber
  if (pathSegments[1] === 'lot' && pathSegments[2]) {
    const lotNumber = pathSegments[2];
    try {
      const lotData = await fetchLotData(lotNumber);
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify(lotData));
    } catch (error) {
      console.error('Error fetching lot data:', error);
      res.writeHead(500, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ error: 'Failed to fetch lot data' }));
    }
  } else {
...
module.exports = {
  handleApiRequest,
  // Also export fetchLotData for direct usage if needed
  fetchLotData: async (lotNumber) => {
    await browserPromise; // Ensure browser is ready
    return fetchLotData(lotNumber);
  }
};
